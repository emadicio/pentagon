...
class Three {
  ...
  _initBackground() {
    ...
    const texture = loader.load(this._scenery.background.image, () => {
  ...
  _initLight() {
    const light = new THREE.SpotLight(...this._scenery.light.params);
    light.position.set(...this._scenery.light.position);
  ...
  _initFloor() {
    ...
    const floorMaterial = new THREE.MeshPhongMaterial({
      ...
      bumpScale: 0.1,
      shininess: this._scenery.floor.shininess,
  ...
  async _initObject() {
    return new Promise((resolve) => {
      const loader = new THREEx.GLTFLoader().setPath(this._scenery.object.path);
      loader.load(this._scenery.object.filename, (gltf) => {
        this._object = gltf.scene;

        this._object.scale.x *= this._scenery.object.scale;
        this._object.scale.y *= this._scenery.object.scale;
        this._object.scale.z *= this._scenery.object.scale;

        this._object.position.set(...this._scenery.object.defaultPosition);
        this._object.rotation.set(...this._scenery.object.defaultRotation);
  ...
  _resetObjectTransform() {
    this._setObjectTargetPositon(...this._scenery.object.defaultPosition);
    this._object.rotation.set(...this._scenery.object.defaultRotation);
  }
